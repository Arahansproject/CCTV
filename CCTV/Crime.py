...
gmaps_key = "AIzaSyBGgt61QlocXPNpuQeoflSNkJ-njRjYR30"
...
crime_anal_police = pd.read_csv(ctx+'crime_in_Seoul_include_gu_name.csv',thousands =',', encoding = 'utf-8')
crime_anal_police.head()

#***********************
# Pivot_table (행열바꾸기 기능) 이용해서 데이터 정리하기
#***********************
...
police['강도검거율'] = police['강도 검거'] / police ['강간 발생']*100
police['살인검거율'] = police['살인 검거'] / police ['강간 발생']*100
police['절도검거율'] = police['절도 검거'] / police ['강간 발생']*100
police['폭력검거율'] = police['폭력 검거'] / police ['강간 발생']*100
...
# min_max_scalar(x)는 최대/최소값이 각각 1, 0이 되도록 스케일링
...
# 인구수와 CCTV 개수, 그리고 살인과 강도사건
# 전체적인 상관계수는 CCTV와 살인의 관계가 
# 낮을지 몰라도(방지효과)
# CCTV가 없을 때 살인이 많이 일어나는 구간이
# 있습니다. 즉, CCTV 개수를 기준으로 좌측면에
...
target_col = ['강간','강도','살인','절도','폭력'] # 범죄추가
...
# 강남구, 양천구, 영등포구가 범죄발생건수가 높습니다.
# 송파구와 서초구도 낮다고 볼 수 없습니다.
# 정말 강남 3구가 안전한가에 대한 의문이 생김.

#police_norm.to_csv(ctx+'./crime_in_Seoul_final.csv', sep = ',', encoding = 'utf-8')
